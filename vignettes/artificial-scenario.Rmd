...
plot(st_geometry(regions),reset=F,col=adjustcolor("pink",.2))
...
for(logmean in c(5,50,500)){
    ...
    plot(st_geometry(frtrs),add=T,col="green",lwd=2)
    layout(1)
    par(mar=c(5,4,2,2))
}
...
```{r regpos,out.width="45%",fig.cap="impact of border position on border detection"}
...
for( crds in coordtest ){
    ...
    for(logmean in c(5,50,500)){
        ...
        plot(density(regions$pop),main=paste("Sample size distribution"))
        plot(regions[,"distrib"],reset=F,main=paste("Frontier detection with median sample size ~",logmean))
        plot(frtrs,add=T,col="green",lwd=2)
        layout(1)
        par(mar=c(5,4,2,2))
    }
}
